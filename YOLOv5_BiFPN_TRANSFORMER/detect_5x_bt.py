...
weights=os.path.join('results_5x_bt/runs/train', 'exp_5x_bt2', 'weights', 'best.pt')
data_source = '/gwhd_dataset/test/images'
detect_output_dir = 'results_5x_bt/runs/detect'
detect_exp_name = 'exp_5x_bt2'
...
with open('detect_5x_bt_log.txt', 'w') as detect_log_file:
    result = subprocess.run(detect_command, stdout=detect_log_file, stderr=subprocess.STDOUT)

if result.returncode != 0:
    print(f"Testing command failed with return code {result.returncode}. See 'detect_5x_bt_log.txt' for details.")
else:
    print("Testing command executed successfully. See 'detect_5x_bt_log.txt' for details.")

print("Testing the Model Completed.")
print("See final metrics in detect_5x_bt_log.txt file")

print("Get final metrics on detected labels:")

predictions_folder = 'results_5x_bt/runs/detect/exp_5x_bt2/labels'
