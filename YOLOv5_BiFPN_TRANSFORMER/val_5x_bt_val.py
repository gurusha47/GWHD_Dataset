...
with open('val_5x_bt_log1_val.txt', 'w') as val_log_file:
    result = subprocess.run(val_command, stdout=val_log_file, stderr=subprocess.STDOUT)

if result.returncode != 0:
    print(f"Validation command failed with return code {result.returncode}. See 'val_5x_bt_log1_val.txt' for details.")
else:
    print("Validation command executed successfully. See 'val_5x_bt_log1_val.txt' for details.")

print("Validating the Model Completed.")
print("See final metrics in val_5x_bt_log1_val.txt file")
