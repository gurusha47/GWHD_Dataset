...
with open('train_5x_bifpn_log2.txt', 'w') as log_file:
    result = subprocess.run(command, stdout=log_file, stderr=subprocess.STDOUT)

if result.returncode != 0:
    print(f"Command failed with return code {result.returncode}. See 'train_5x_bifpn_log2.txt' for details.")
else:
    print("Command executed successfully. See 'train_5x_bifpn_log2.txt' for details.")
